import { text } from "./privacy.ru";

export const RU = {
    Main: {
        Start: {
            implemented: 'Реальное ICO',
            content: 'Реальные ставки на Bitcoin',
            loading: 'Загрузка 50 %',
            btnUp: 'Курс вверх',
            btnDown: 'Курс вниз'
        },
        Modal: {
            title: {
                first: 'Отличная работа',
                // second: 'Join now!'
            },
            logIn: 'Войти',
            signUp: 'Регистрация'
        }
    },

    Auth: {
        Login: {
            title: 'Авторизация',
            phoneTitle: 'Телефон',
            phone: '063 111 22 33',
            password: 'Пароль',
            forgotPassword: 'Забыли пароль?',
            loginIn: 'ВОЙТИ',
            or: 'или',
            signUp: 'РЕГИСТРАЦИЯ',
            support: 'Поддержка'
        },

        Register: {
            title: 'Регистрация',
            nameTitle: 'Имя',
            name: 'John Lucky',
            phoneTitle: 'Телефон',
            phone: '063 111 22 33',
            emailTitle: 'Почта',
            email: 'lucky@mail.com',
            password: 'Пароль',
            passwordRepeat: 'Повторите пароль',
            signUp: 'ЗАРЕГИСТРИРОВАТЬСЯ',
            support: 'Поддержка',
            promo: 'Промокод(необязательно)',
            begin: 'Мне больше 18 лет',
            legal: 'Юридические положения',
            and: 'и',
            privacy: 'Политика конфиденциальности',
            text
        },

        ForgotPassword: {
            title: 'Забыли пароль?',
            emailTitle: 'Почта',
            email: 'lucky@mail.com',
            reset: 'Отправить ссылку',
            support: 'Поддержка'
        },

        StatusInfo: {
            title: 'Ссылка отправлена',
            statusContent: {
                sendLinkContent: 'Мы отправили ссылку на вашу почту ',
                checkEmailContent: 'Пожалуйста, проверьте свою электронную почту и нажмите на ссылку'
            },
            ok: 'На главную',
            support: 'Поддержка'
        },

        ResetPassword: {
            title: 'Введите секретный код',
            passwordNew: 'Пароль',
            passwordRepeat: 'Повторите пароль',
            reset: 'Сохранить',
            passwordChanged: 'Пароль изменен!',
            ok: 'OK',
            support: 'Поддержка'
        },

        Support: {
            title: 'Поддержка',
            writeQuestion: 'Ваш вопрос',
            send: 'ОТПРАВИТЬ',
            support: 'Поддержка'
        }
    },

    Training: {
        WinningAndLosing: {
            Winning: {
                title: 'Поздравляем',
                youWon: 'Вы выиграли',
                addToWallet: 'Добавить в кошелек',
                withdraw: 'Вывести'
            },

            Losing: {
                title: 'Вы на правильном пути',
                btnTryAgain: 'Попробовать еще'
            }
            
        },

        UsualState: {

            Chart: {
                online: 'онлайн'
            },

            BetsInProgress: {
                title: 'Сделанные ставки'
            },

            MakeBet: {
                title: 'Сделайте ставку',
                betSize: 'Размер ставки',
                yourProfit: 'Ваша прибыль',
                btnUp: 'Курс вверх',
                btnDown: 'Курс вниз'
            },

            DemoWallet: {
                title: 'Демо счёт',
                nameTitle: 'Имя',
                name: 'Test',
                winsTitle: 'Выиграши',
                wins: '0',
                balanceTitle: 'Баланс',
                // balance: '',
                lastWinTitle: 'Последний выиграш',
                btnBetBitcoin: 'Перейти на реальный',
                btnPlayWithFriends: 'Играть с друзьями'
            },

            SwitchingToReal: {
                title: 'Мой Bitcoin кошелек',
                btnSwitchToReal: 'Перейти в Реальный',
                btnContinueDemo: 'Продолжить Демо'
            },

            SwitchingToDemo: {
                title: 'Мой Bitcoin кошелек',
                btnSwitchToDemo: 'Перейти в Демо',
                btnContinueReal: 'Продолжить Реальный'
            }

        },

        ThreeWinning: {
            title: 'Отличная работа',
            content: 'Вы выиграли 3 раза подряд',
            yourWinnings: 'Ваш выигрыш составил',
            btnInvest: 'Инвестировать',
            btnTrain: 'Тренироваться с нами'
        },

        InviteFriends: {

            Form: {
                title: 'Пригласить друзей',
                content: 'Вы получите 1 BTC на демо-кошелек за каждую регистрацию вашего друга',
                sendInvitation: 'Отправить приглашение',
                emailPlaceholder: 'hi@gmail.com',
                btnSendInvite: 'Отправить',
                shareLinkTitle: 'Поделиться ссылкой',
                shareLink: 'bitcybets.com/inviting',
                copyLink: 'Скопировано',
                btnCopyShareLink: 'Скопировать'
            },

            Success: {
                title: 'Приглашение отправлено',
                content: 'Спасибо за активность, вы получаете',
                btnTrain: 'Тренироваться с нами'
            }
            
        }
    },

    Online: "Онлайн",

    FulfillingRealMoney: {

        CurrencyExchange: {
            title: 'Варианты пополнения',
            amount: 'Количество',
            btnDeposit: 'Пополнить',
        },

        USD: {
            title: 'Оплата долларами',
            optionNotReady: 'В данный момент не работает',
            content: 'Введите данные вашей банквоской карты',
            cardNumber: 'Номер карты',
            expiring: 'Истекает',
            CVC: 'CVC',
            holdersNameTitle: 'Имя и фамилия владельца',
            holdersName: 'Михаил Васкес',
            deposit: 'Deposit'
        },

        BTC: {
            title: 'Оплата Bitcoin',
            OurBTCWallet: 'Наш Биткйон кошелек',
            BTCWalletAddress: 'Наш BTC кошелек',
            BTCWalletPlaceholder: '161PtRedhQgxoc3UXgN6iVqMF3XFVs96QV',
            btnCopyLink: 'Скопировать',
            linkCopy: 'Скопировано',
            paymentScreenshotTitle: 'Загрузите скриншот оплаты',
            paymentScreenshotDrag: 'Перетащите файл сюда или',
            paymentScreenshotChoose: 'ВЫберите файл',
            transactionIDTitle: 'Номер транзакции',
            transactionIDPlaceholder: '11223344',
            btnSend: 'Send',
            emptyData: 'Пустые данные',
            emptyID: 'Пустой идентификатор транзакции'
        },

        CompletionNotification: {
            title: 'Оплата завершена',
            content: 'Удачи в ваших ставках',
            btnGoToBets: 'Перейти к ставкам'
        }
    },

    BettingRealMoney: {

        WinningAndLosing: {
            Winning: {
                title: 'Поздравляем',
                youWon: 'Вы выиграли',
                btnAddToWallet: 'Добавить в кошелек',
                btnWithdraw: 'Вывести'
            },

            Losing: {
                title: 'Вы на правильном пути',
                btnTryAgain: 'Попробовать еще'
            }
            
        },

        UsualState: {

            Chart: {
                online: 'онлайн'
            },

            BetsInProgress: {
                title: 'Сделанные ставки'
            },

            MakeBet: {
                title: 'Сделайте ставку',
                betSize: 'Размер ставки',
                yourProfit: 'Ваша прибыль',
                btnUp: 'Курс вверх',
                btnDown: 'Курс вниз'
            },

            MyWallet: {
                title: 'Кошелек',
                nameTitle: 'Имя',
                name: 'Test',
                winsTitle: 'Выигрыши',
                balanceTitle: 'Баланс',
                lastWinTitle: 'Последний',
                btnDeposit: 'Пополнить',
                btnWithdraw: 'Вывести'
            }

        },

        WithdrawalMethod: {
            title: 'Снятие',
            BTCWalletTitle: 'Ваш БКТ кошелек',
            BTCWalletPlaceholder: '1BvBMSEYstWetqTFn5Au4m4GFg7xJaNVN2',
            AmountBTCTitle: 'Сумма БКТ',
            AmountBTC: '0.125',
            btnWithdraw: 'СНЯТЬ'
        },

        CompletionNotification: {
            title: 'Завершено',
            content: 'Удачи',
            btnStartBetting: 'Начать ставки'
        }
            
    },

    ModalWindows: {

        LogOut: {
            title: 'Вы уверены?',
            btnLogOut: 'Выйти',
            btnContunue: 'Продолжить'
        },

        OutOfMoney: {
            
            Demo: {
                title: 'Не хватает денег',
                btnBetRealBTC: 'Поставить Bitcoin',
                btnRestart: 'Перезапустить Демо'
            },

            Real: {
                title: 'Не хватает Bitcoin',
                deposit: 'Пополнить',
                btnRestart: 'Перезапустить Демо'
            }
        }
    },
    Menu: {
        first: "Разместить рекламу",
        second: "Мои объявления",
        third: "Вступление",
        exit: "Выход"
    },

    Intro: {
        Step1: {
            title: 'График',
            content: 'На графике вы можете наблюдать  изменения цены BTC'
        },
        Step2: {
            title: 'Банк',
            content: 'Здесь вы можете увидеть, сколько игроков делают ставки и сколько стоит банк'
        },
        Step3: {
            title: 'Ваша ставка',
            content: 'Здесь вы можете сделать ставку  и установить количество BTC'
        },
        Step4: {
            title: 'Кошелек',
            content: 'Здесь вы можете следить за своим балансом, менять кошелек с демо на настоящий, приглашать друзей, вносить и выводить BTC'
        },
        ClickOnWallet: {
            title: 'Нажмите на кошелек',
            content: 'Вы перейдете на другую вкладку'
        },
        Step5: {
            title: 'Приглашение друзей',
            content: 'Здесь вы можете пригласить своих друзей, чтобы повеселиться и заработать больше BTC'
        },
        btnIntro: {
            prev: 'Назад',
            skip: 'Пропустить',
            next: 'Далее',
            finish: 'Завершить'
        }
    },

    Ads: {
        MyAds: {
            title: 'My ads',
            current: 'Current',
            finished: 'Finished',
            notAds: 'You have not run any ads yet'
        },
        MyAd: {
            selectBanner: 'Select a banner to add',
            chooseFile: 'Choose file',
            audience: 'Audience',
            country: 'Country',
            timeZone: 'Time zone',
            time: 'Time',
            startTime: 'Started',
            finishTime: 'Finished',
            totalTime: 'Totel time',
            result: 'Results',
            display: 'Displays',
            clicks: 'Clicks',
            budget: 'Budget',
            changeBtn: 'Change',
            repeatBtn: 'Repeat'
        },
        Wallet: {
            title: 'My wallet',
            name: 'Name',
            balance: 'Balance',
            deposite: 'Deposit'
        }
    },
    
    support: 'Нужна помощь?'
};
